...
export const chatAPI = {
  ...
  async sendMessage(data, chatId) {
    try {
      ...
      if (!response.body) {
        throw new Error('当前浏览器不支持流式响应，无法读取服务端输出流')
      }
  ...
  // 发送客服消息
  async sendServiceMessage(prompt, chatId, options = {}) {
    try {
      const response = await fetch(`${BASE_URL}/ai/record-contract-agent?prompt=${encodeURIComponent(prompt)}&chatId=${chatId}`, {
        method: 'GET',
        headers: {
